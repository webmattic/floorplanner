...
export const PanelIntegrationTest: React.FC = () => {
  ...
  const { metrics, getPerformanceStatus, getOptimizationRecommendations } = usePanelPerformance();
    ...
    const suites: TestSuite[] = [
      {
        ...
        progress: 0,
        tests: [
          { id: "show-panel", name: "Show Panel", status: "pending", duration: 0 },
          { id: "hide-panel", name: "Hide Panel", status: "pending", duration: 0 },
          { id: "toggle-panel", name: "Toggle Panel", status: "pending", duration: 0 },
          { id: "minimize-panel", name: "Minimize Panel", status: "pending", duration: 0 },
          { id: "maximize-panel", name: "Maximize Panel", status: "pending", duration: 0 },
        ],
      },
      {
        ...
        progress: 0,
        tests: [
          { id: "move-panel", name: "Move Panel", status: "pending", duration: 0 },
          { id: "resize-panel", name: "Resize Panel", status: "pending", duration: 0 },
          { id: "snap-to-edges", name: "Snap to Edges", status: "pending", duration: 0 },
          { id: "boundary-constraints", name: "Boundary Constraints", status: "pending", duration: 0 },
        ],
      },
      {
        id: "panel-grouping",
        ...
        progress: 0,
        tests: [
          { id: "create-group", name: "Create Group", status: "pending", duration: 0 },
          { id: "remove-group", name: "Remove Group", status: "pending", duration: 0 },
          { id: "group-operations", name: "Group Operations", status: "pending", duration: 0 },
        ],
      },
      {
        ...
        progress: 0,
        tests: [
          { id: "save-state", name: "Save Panel State", status: "pending", duration: 0 },
          { id: "restore-state", name: "Restore Panel State", status: "pending", duration: 0 },
          { id: "workspace-presets", name: "Workspace Presets", status: "pending", duration: 0 },
        ],
      },
      {
        id: "performance",
        ...
        progress: 0,
        tests: [
          { id: "render-performance", name: "Render Performance", status: "pending", duration: 0 },
          { id: "memory-usage", name: "Memory Usage", status: "pending", duration: 0 },
          { id: "frame-rate", name: "Frame Rate", status: "pending", duration: 0 },
          { id: "stress-test", name: "Stress Test", status: "pending", duration: 0 },
        ],
      },
      {
        id: "integration",
        ...
        progress: 0,
        tests: [
          { id: "canvas-integration", name: "Canvas Integration", status: "pending", duration: 0 },
          { id: "store-integration", name: "Store Integration", status: "pending", duration: 0 },
          { id: "keyboard-shortcuts", name: "Keyboard Shortcuts", status: "pending", duration: 0 },
          { id: "error-handling", name: "Error Handling", status: "pending", duration: 0 },
        ],
      },
    ];
    ...
    try {
      switch (`${suiteId}-${testId}`) {
        case "panel-basic-show-panel":
          ...
          if (!panels.drawingTools?.isVisible) throw new Error("Panel not shown");
          break;

        case "panel-basic-hide-panel":
          hidePanel("drawingTools");
          if (panels.drawingTools?.isVisible) throw new Error("Panel not hidden");
          break;

        case "panel-basic-toggle-panel":
          const initialState = panels.drawingTools?.isVisible;
          togglePanel("drawingTools");
          if (panels.drawingTools?.isVisible === initialState) throw new Error("Panel not toggled");
          break;

        case "panel-basic-minimize-panel":
          showPanel("drawingTools");
          minimizePanel("drawingTools");
          if (!panels.drawingTools?.isMinimized) throw new Error("Panel not minimized");
          break;

        case "panel-basic-maximize-panel":
          minimizePanel("drawingTools");
          maximizePanel("drawingTools");
          if (panels.drawingTools?.isMinimized) throw new Error("Panel not maximized");
          break;
        ...
        case "panel-grouping-create-group":
          const groupId = createGroup("Test Group", ["drawingTools", "properties"]);
        ...
        case "performance-render-performance":
          ...
          if (renderTime > 100) throw new Error(`Slow rendering: ${renderTime.toFixed(2)}ms`);
          break;

        case "performance-memory-usage":
          if (metrics.memoryUsage > 200) {
            throw new Error(`High memory usage: ${metrics.memoryUsage.toFixed(2)}MB`);
          }
          break;
        ...
        case "integration-store-integration":
          ...
          if (!storeState.panels || Object.keys(storeState.panels).length === 0) {
  ...
  }, [panels, showPanel, hidePanel, togglePanel, minimizePanel, maximizePanel, updatePanelPosition, updatePanelSize, createGroup, metrics]);
  ...
  const runAllTests = useCallback(async () => {
    ...
    for (const suite of testSuites) {
      ...
      for (const test of suite.tests) {
        ...
        setTestResults(prev => ({ ...prev, [`${suite.id}-${test.id}`]: result }));
        ...
        setTestSuites(prev => prev.map(s => 
          s.id === suite.id ? { ...s, progress: suiteProgress, tests: suiteResults } : s
        ));
                        ...
                        return (
                          <div key={test.id} className="flex items-center justify-between p-2 border rounded">
                            <div className="flex items-center gap-2">
                              ...
                              {result?.status === "skipped" && <AlertTriangle className="h-4 w-4 text-yellow-500" />}
                              {result?.status === "running" && <RefreshCw className="h-4 w-4 animate-spin text-blue-500" />}
                              {!result && <div className="h-4 w-4 rounded-full bg-gray-300" />}
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                <div className="flex justify-between">
                  ...
                  <Badge variant={metrics.frameRate >= 50 ? "default" : "destructive"}>
                    {metrics.frameRate} fps
                  </Badge>
                </div>
                <div className="flex justify-between">
                  <span>Render Time:</span>
                  <Badge variant={metrics.renderTime < 16 ? "default" : "destructive"}>
                    {metrics.renderTime.toFixed(2)} ms
                  </Badge>
                </div>
                <div className="flex justify-between">
                  <span>Memory Usage:</span>
                  <Badge variant={metrics.memoryUsage < 100 ? "default" : "destructive"}>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-center">
                  <div className={`text-3xl font-bold text-${performanceStatus.color}-600`}>
