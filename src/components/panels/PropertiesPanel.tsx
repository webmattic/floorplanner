...
export const PropertiesPanel: React.FC<{ panelId?: string }> = ({
  panelId = "properties",
}) => {
  // Zustand selectors
  const selectedElementsRaw = useFloorPlanStore((s) => s.selectedElements);
  const selectedElements = Array.isArray(selectedElementsRaw)
    ? selectedElementsRaw
    : [];
  ...
  // Use default layers since layers is not in the store yet
        ...
        <form className="space-y-4" aria-label="Room Properties">
          <div className="grid grid-cols-2 gap-2">
            ...
            <Input
              ...
              onChange={(e) =>
                updateRoom(selected.id, { label: e.target.value })
              }
            />
            ...
            <Input
              ...
              value={elementData.y}
              onChange={(e) =>
                updateRoom(selected.id, { y: Number(e.target.value) })
              }
            />
            ...
            <Input
              ...
              onChange={(e) =>
                updateRoom(selected.id, { height: Number(e.target.value) })
              }
            />
            ...
            <Input
              ...
              onChange={(e) =>
                updateRoom(selected.id, { color: e.target.value })
              }
            />
            <Label htmlFor="texture">Texture</Label>
            <Select
              value="solid"
              onValueChange={(val) => console.log("Texture change:", val)}
              aria-label="Texture"
            >
            ...
            <Select
              value={layers[0].id}
              onValueChange={(val) => console.log("Layer change:", val)}
            ...
            <div className="flex gap-2">
              {MATERIAL_SWATCHES.map((swatch) => (
                <button
                  ...
                  onClick={() =>
                    updateRoom(selected.id, { color: swatch.color })
                  }
                />
              ))}
            </div>
    ...
    if (selected.type === "wall") {
      return (
        <form className="space-y-4" aria-label="Wall Properties">
          <div className="grid grid-cols-2 gap-2">
            <Label htmlFor="wall-type" className="font-semibold col-span-2">
              Wall Type
            </Label>
            <Label htmlFor="color">Color</Label>
            <Input
              ...
              value={elementData.color}
              onChange={(e) =>
                updateWall(selected.id, { color: e.target.value })
              }
            />
            ...
            <Slider
              ...
              min={4}
              max={24}
              value={[elementData.thickness]}
              onValueChange={(val) =>
                updateWall(selected.id, { thickness: val[0] })
              }
            />
        ...
        <form className="space-y-4" aria-label="Furniture Properties">
          <div className="grid grid-cols-2 gap-2">
            ...
            <Input
              ...
              value={elementData.label}
              onChange={(e) =>
                updateFurniture(selected.id, { label: e.target.value })
              }
            />
            <Label htmlFor="x">X</Label>
            <Input
              ...
              value={elementData.x}
              onChange={(e) =>
                updateFurniture(selected.id, { x: Number(e.target.value) })
              }
            />
            <Label htmlFor="y">Y</Label>
            <Input
              ...
              value={elementData.y}
              onChange={(e) =>
                updateFurniture(selected.id, { y: Number(e.target.value) })
              }
            />
            <Label htmlFor="width">Width</Label>
            <Input
              ...
              value={elementData.width}
              onChange={(e) =>
                updateFurniture(selected.id, { width: Number(e.target.value) })
              }
            />
            <Label htmlFor="height">Height</Label>
            <Input
              ...
              value={elementData.height}
              onChange={(e) =>
                updateFurniture(selected.id, { height: Number(e.target.value) })
              }
            />
            <Label htmlFor="color">Color</Label>
            <Input
    ...
    <FloatingPanel panelId={panelId}>
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="properties" role="tab" aria-label="Properties">
            Properties
          </TabsTrigger>
          <TabsTrigger value="layers" role="tab" aria-label="Layers">
            Layers
          </TabsTrigger>
        </TabsList>
        <TabsContent value="properties" forceMount>
          ...
          <div
            ...
            data-testid="selection-status-area"
          >
            <div className="text-xs text-muted-foreground">
              ...
              {selectedElements.length === 0 ? (
                <div
                  ...
                  data-testid="no-selection-status"
                >
                  <span>
                    Select objects on the canvas to edit their properties
                  </span>
                </div>
              ) : (
                <div
